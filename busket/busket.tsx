...
import { TextInput, Button, Switch, Group, Modal, createStyles } from '@mantine/core';
...
const BasketModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
  ...
  return (
    <Modal opened={isOpen} onClose={onClose} title="Basket" classNames={{ modal: classes.modal }}>
        ...
        <Group position="center" style={{ marginBottom: '20px' }}>
          <Button onClick={() => setShowForm(!showForm)}>{showForm ? 'Hide Form' : 'Ready to Buy'}</Button>
        </Group>
        {showForm && (
          <>
            <TextInput
        ...
        <style jsx>{`
          ...
          .basket-content {
            max-height: 70vh;
            overflow-y: auto;
            padding-right: 10px; /* To ensure padding doesn't disappear due to scrollbar */
          }
        `}</style>
      </div>
    </Modal>
