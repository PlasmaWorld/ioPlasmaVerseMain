...
const VrmViewer = () => {
  ...
  const applyTextureOverride = async (category, overrideThumbnail) => {
    ...
    if (vrm) {
        // Apply textures based on how many we have
        vrm.scene.traverse((child) => {
            if (child.isMesh) {
                const meshName = child.name;

                // Determine the correct texture for this mesh
                let texturePath = null;

                if (category === 'body' && texturePaths.length >= 3) {
                    // Map specific textures to Body, Head, and Mouth
                    if (meshName === 'Body') texturePath = texturePaths[0];
                    if (meshName === 'Headbaked(copy)') texturePath = texturePaths[1];
                    if (meshName === 'Headbaked(copy)_1') texturePath = texturePaths[2];
                } else if (texturePaths.length === 1) {
                    // For categories with only one texture, apply it to the entire category
