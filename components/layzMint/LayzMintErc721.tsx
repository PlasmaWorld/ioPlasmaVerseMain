...
import { Chain, defineChain, getContract, prepareContractCall, PreparedTransaction, readContract, resolveMethod, ThirdwebContract } from 'thirdweb';
...
interface ClaimComponentProps {
  ...
  const LayzMintComponentErc721: React.FC<ClaimComponentProps> = ({ contractAddress, chainId }) => {
    ...
    const prepareTransaction = async (): Promise<PreparedTransaction<any>> => {
      ...
      const resolvedMethod = await resolveMethod("lazyMint");
      console.log("Resolved method:", resolvedMethod);
    
      // Fetch the next token ID to mint
      const result = await nextTokenIdToMint({ contract });
      const nextTokenToMint = Number(result);
      console.log("Next token to Mint:", nextTokenToMint);
      ...
      try {
        ...
        if (selectedFile) {
          ...
          console.log("Image uploaded to IPFS:", ipfsUrlImage);
        }
    
        // Upload the video file to IPFS
        if (selectedFileVideo) {
          ...
          console.log("Video uploaded to IPFS:", ipfsUrlVideo);
        }
    
        // Upload the VRM file to IPFS
        if (vrmFile) {
          console.log("Uploading VRM file to IPFS...");
          ...
          ipfsUrlVrm = `${vrmUri}`;
          console.log("VRM file uploaded to IPFS:", ipfsUrlVrm);
        }
        ...
        console.log("Metadata object created:", metadataObject);
    
        const metadataString = JSON.stringify(metadataObject);
        const metadataBlob = new Blob([metadataString], { type: 'application/json' });
    
        // Naming the metadata file with the next token ID as part of the path
        const metadataFileName = `metadata.json.${nextTokenToMint}`;
        console.log("Naming metadata file:", metadataFileName);
        ...
        // Get the base IPFS URL without the token ID at the end
        let ipfsUrl = `${metadataUri}`;
    
        // Correct the IPFS URL by removing the extra token ID or path components
        ipfsUrl = ipfsUrl.replace(`${nextTokenToMint}`, "");
        console.log("Final IPFS URL to be used for minting:", ipfsUrl);
    
        // Prepare the contract call with the correct IPFS URL
    ...
    return (
      <div className={styles.MintApp}>
        <div className={styles.statusHeader}>
          <h2>LayzMint Your unique NFT. You can add Images, videos, and 3D Files</h2>
        </div>
  ...
  export default LayzMintComponentErc721;