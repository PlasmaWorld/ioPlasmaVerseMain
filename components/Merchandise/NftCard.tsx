...
import { getActiveClaimCondition, nextTokenIdToMint } from "thirdweb/extensions/erc1155";
import { ioUSDCondract, Merchendise } from "@/const/contracts";
import { NftInformationPrice } from "./NftPrice";
...
export const NFTCard: FC<INFTCardProps> = ({ tokenId }) => {
  ...
  useEffect(() => {
    const fetchNFTAndClaimCondition = async () => {
      try {
        const contract = getContractInstance("0xa8d80A16dAFC559Bf2af9e6e295b412960A49626");
  ...
  const formattedDescription = `
    <p>T-shirts are the hidden champions of your wardrobe. There&apos;s nothing easier to combine than our classic crew neck T-shirt. It looks elegant under a shirt or worn on its own.</p>
    <ul>
      <li>Double seams</li>
      <li>Side seams</li>
      <li>Single jersey</li>
      <li>100% cotton</li>
      <li>Modern fit</li>
      <li>Fabric weight: 160 g/m²</li>
      <li>Neck tape</li>
      <li>Heather grey: 93% cotton, 7% viscose</li>
    </ul>
    <p>Measurements (Length x Width in cm):</p>
    <ul>
      <li>S = 68 x 48</li>
      <li>M = 70 x 51</li>
      <li>L = 73 x 54</li>
    ...
    <p>For maximum durability, we recommend the following care instructions:</p>
    <ul>
      <li>Wash before first wear</li>
      <li>Maximum wash temperature 30°C</li>
      <li>Do not overfill the washing machine on the first wash</li>
      <li>Do not use liquid detergents</li>
      <li>Do not bleach</li>
      <li>Do not use fabric softener</li>
      <li>Do not tumble dry hot</li>
  ...
  return (
    <div className="relative flex flex-col items-center p-4 bg-opacity-10 bg-white rounded-lg shadow-md transition duration-300 ease-in-out hover:bg-opacity-20 h-[450px] w-[300px] m-2 cursor-pointer" onClick={() => setShowInfo(true)} role="button" aria-label="Show more info">
      ...
      {currentNFT ? (
        <>
          <MediaRenderer src={currentNFT.metadata.image} client={client} className="w-full h-52 object-cover rounded-md" />
            ...
            <Container maxWidth="md">
              <div {...handlers} className="fixed inset-0 bg-gray-900 bg-opacity-75 flex justify-center items-start pt-20 overflow-auto z-50 sm:items-center mt-14">
                <div className="container max-w-3xl mx-auto p-9 rounded-lg shadow-lg mb-20 bg-[#181818] max-h-[80vh] overflow-y-auto">
                  <p>This Merchandise is from the ioPlasmaWorld Collection.</p>
                  
                  <div className="mt-4">
                    {currentNFT?.metadata ? (
                      <MediaRenderer src={currentNFT.metadata.image} client={client} className="w-full h-64 object-cover rounded-md" />
                    ) : (
                      <p className="text-white">NFT metadata not available.</p>
                    )}
                  </div>
                  <div className="mt-4 text-white">
                    <p className="text-2xl font-bold">You&apos;re about to Buy the following item from the Merchandise Shop.</p>
                    <p className="mt-2">This sale Starts at: {startTime()}</p>
                    <p className="mt-2">Items claimed: {claimedSupply()} </p>
                    <p className="mt-2 text-lg font-semibold">Name #{currentNFT?.metadata.name}</p>
                  </div>
                  <div className="mt-4">
                    <h3 className="text-white text-xl font-bold">Description</h3>
                    <div className="text-white" dangerouslySetInnerHTML={{ __html: formattedDescription }} />
                  </div>
                  {size && account && (

                    <>
                      <Button onClick={addToBasket} style={{ marginTop: '20px' }}>
                        Add to Basket
                      </Button>
                    </>
                  )}
                  <Select
                    ...
                    data={[
                      { value: 'S', label: 'S = 68 x 48' },
                      { value: 'M', label: 'M = 70 x 51' },
                      { value: 'L', label: 'L = 73 x 54' },
                      { value: 'XL', label: 'XL = 75 x 57' },
                      { value: '2XL', label: '2XL = 78 x 60' }
                    ]}
                    value={size}
