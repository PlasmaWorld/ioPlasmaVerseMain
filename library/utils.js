  ...
  meshes.forEach((mesh) => {
    ...
    if (mat.type == 'ShaderMaterial') {
      if (mat.transparent == true) {
        ...
        mToonMesh.push(mesh);
        if (mat.uniforms.alphaTest?.value != 0) requiresTransparency = true;
      }
    } else {
      if (mat.transparent == true) {
        ...
        stdMesh.push(mesh);
        if (mat.alphaTest != 0) requiresTransparency = true;
      }
    }
  });
...
export function getMaterialsSortedByArray(meshes) {
  ...
  meshes.forEach((mesh) => {
    ...
    mats.forEach((mat) => {
      if (mat.type == 'ShaderMaterial') {
        if (mat.transparent == true) mToonTranspMats.push(mat);
        else if (mat.uniforms.alphaTest.value != 0) mToonCutoutMats.push(mat);
        else mToonMats.push(mat);
      } else {
        if (mat.transparent == true) stdTranspMats.push(mat);
        else if (mat.alphaTest != 0) stdCutoutpMats.push(mat);
        else stdMats.push(mat);
      }
    });
  });
...
export async function getCroppedScreenshot(elementId, posX, posY, width, height, debug = false) {
  ...
  return await html2canvas(snapShotElement).then(async function (canvas) {
    ...
    const base64Data = Buffer.from(newdataurl.replace(/^data:image\/\w+;base64,/, ''), 'base64');
  ...
  if (Array.isArray(cameraPosition)) {
    ...
    positionString.forEach((pos) => {
      ...
      switch (pos) {
        ...
        default:
          console.warn(
            'unkown cameraPosition name: ' + pos + ' in: ' + cameraPosition + '. Please use left, right, bottom, top, back or front'
          );
          break;
      }
    });
...
export const createBoneDirection = (skinMesh) => {
  ...
  for (let i = 0; i < skinMesh.skeleton.bones.length; i++) {
    if (skinMesh.skeleton.bones[i].name.includes('Shoulder')) {
      ...
      bonesArrange[i] = 1;
    else if (skinMesh.skeleton.bones[i].name.includes('Foot') || skinMesh.skeleton.bones[i].name.includes('Toes'))
      bonesArrange[i] = 3;
  ...
  for (let f = 0; f < bnIdx.length / 4; f++) {
    ...
    vertexPosition.set(pos[idxPosBase], pos[idxPosBase + 1], pos[idxPosBase + 2]); //z
    ...
    if (bonesArrange[highIdx] !== 3) dir.subVectors(vertexPosition, boneTargetPos).normalize();
    else dir.set(normals[idxPosBase], normals[idxPosBase + 1], normals[idxPosBase + 2]); //z
...
export const renameVRMBones = (vrm) => {
  ...
  if (bones['leftShoulder'] == null) {
    if (
      bones['leftUpperArm'].node.parent != bones['chest']?.node &&
  ...
  if (bones['rightShoulder'] == null) {
    if (
      bones['rightUpperArm'].node.parent != bones['chest']?.node &&
